...
function cargarMapaYDatos(latitudInicial, longitudInicial, zoomInicial, url, ciudad) {
    // Crear un nuevo mapa con Leaflet
    var map = L.map('map').setView([latitudInicial, longitudInicial], zoomInicial);
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        data.puntos_con_areas.forEach(puntoConArea => {
          ...
          var marker = L.marker([puntoConArea.latitud, puntoConArea.longitud], { icon: customIcon }).addTo(map);
          ...
          marker.bindPopup('<b>' + puntoConArea.titulo + '</b><br>' + puntoConArea.descripcion + '<br><a href="' + puntoConArea.enlace + '">Enlace</a>');
        });
      })
    ...
    if (ciudad) {
      ...
      var urlGeocoding = "https://nominatim.openstreetmap.org/search?format=json&q=" + encodeURIComponent(ciudad);
      ...
      fetch(urlGeocoding)
        ...
        .then(data => {
          ...
          if (data.length > 0) {
            ...
            window.dispatchEvent(new CustomEvent('ciudadCargada', { detail: { latitud: latitud, longitud: longitud } }));
          } else {
      ...
      window.dispatchEvent(new CustomEvent('ciudadCargada', { detail: { latitud: 0, longitud: 0 } }));
    }
  }
  ...
  // Definir las coordenadas iniciales y el zoom del mapa
  var latitudInicial = 41.390189;
  var longitudInicial =  2.166076;
  var zoomInicial = 10;
  
  // Cargar el mapa y los datos del JSON
  cargarMapaYDatos(latitudInicial, longitudInicial, zoomInicial, 'https://raw.githubusercontent.com/j0rd1s3rr4n0/api/master/wifimap/datos.json', ciudad);
